...
//	""", nativeQuery = true)
    @Query(value = """
            	SELECT
            		sub.name AS name,
            		sub.total_score AS sumOfScore,
            		sub.message_uuid AS messageUUID,
            		sub.clarity_score     AS clarityScore,
            		sub.specificity_score AS specificityScore,
            		sub.format_score      AS formatScore,
            		sub.safety_score      AS safetyScore
            	FROM (
            		SELECT
            			u.name AS name,
            			sc.total_score AS total_score,
            			m.message_uuid AS message_uuid,
            			sc.clarity_score     AS clarity_score,
            			sc.specificity_score AS specificity_score,
            			sc.format_score      AS format_score,
            			sc.safety_score      AS safety_score,
            			ROW_NUMBER() OVER (
