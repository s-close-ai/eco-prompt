...
public interface MessageJpaRepository extends JpaRepository<Message, Long> {
	...
	@Query(value = """
		SELECT
			COALESCE(AVG(sc.clarity_score), 0)      AS sc_ec_1,
			COALESCE(AVG(sc.specificity_score), 0)  AS sc_ec_2,
			COALESCE(AVG(sc.format_score), 0)       AS sc_ec_3,
			COALESCE(AVG(sc.safety_score), 0)       AS sc_ec_4
		FROM message m
